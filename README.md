...
1. 用浏览器登录 https://xiaoheihe.cn/
2. 打开开发者工具（F12）→ Network（网络）标签
3. 刷新页面，找到任意一个请求
4. 在请求头中复制完整的 `Cookie` 值
5. 粘贴到应用设置页的小黑盒 Cookie 输入框

### 测试连接返回「请登录后使用该功能」

Cookie 已失效，请重新登录小黑盒并复制最新的 Cookie。

### 静态站打开后图片加载失败

如果某张图片本地不存在，静态站会自动回退使用原始远程 URL。如果远程图片也加载失败，可能是图片链接已过期或网络原因。

### 评论抓取提示风控/验证码

小黑盒有反爬机制，抓取遇到风控时会自动指数退避重试。如果频繁失败，可以稍后再试，或者减少抓取频率。

## 安全说明

- 所有凭证（小黑盒 Cookie、OpenAI API Key）仅保存在服务端数据库中，不会上传到任何第三方
- 导出的静态站点数据**不包含**任何凭证信息
- 本工具仅用于个人收藏数据的备份和离线浏览，请遵守相关平台的使用条款
